...
function filterByCountry(users, nat) {
  return users.filter((user) => user.nat === nat);
}

const filteredUsers = filterByCountry(users, "NO");
...
function filterByCountry(userList, countryCode) {
  let filteredUsers = userList.filter((user) => user.nat === countryCode);
...
// solution 2
console.log(filterToEmails(users));

function filterToEmails(userList) {
  let emailList = [];
  userList.forEach((user) => {
